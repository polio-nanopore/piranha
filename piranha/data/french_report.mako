  ...
  function exportImagePNG(buttonID,svgID,name){
          ...
          <tbody>
            % for row in data_for_report["summary_table"]:
                %for col in config["summary_table_header"]:
                  ...
                  %endif
                %endfor
                <td><a download href="published_data/${this_barcode}/${this_barcode}.consensus.fasta" style="color:${themeColor}"><strong>Télécharger FASTA</strong></a></td>
              </tr>
            % endfor
          </tbody>
        </table>
    ...
    <button class="accordion">Exporter le tableau:</button>
      ...
      <table class="display nowrap" id="myTable3">
        <thead>
          <tr>
              <th style="width:30%;">Séquences identiques</th>
              <th style="width:70%;">ID de séquence</th>
          </tr>
        </thead>
        <tbody>
          <% set_count = 0 %>
      ...
      % if show_control_table:
        <div class="pagebreak"> </div>
        <h3><strong>Tableau 4</strong> | Les contrôles </h3>
          <table class="table">
